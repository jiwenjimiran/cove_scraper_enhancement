...
export function createBetterScrapersController({document,location,MutationObserver,settingsProvider,delay=sleep}){
  ...
  function start(){injectStyles(document);observer=new MutationObserver(schedule);observer.observe(document.documentElement,{attributes:true,childList:true,subtree:true});ensureControls();}
  ...
  function ensureControls(){
    ...
    const tagger=findVideoTagger(document);if(!tagger)return cleanup();const existing=tagger.toolbar.querySelector(`#${CONTROLS_ID}`);if(existing){syncSelectedButton(existing,tagger.list);return;}
    ...
    syncSelectedButton(controls,tagger.list);
  }
  ...
  function runScrapeAll(controls){return runScrape(controls,false);}
  function runScrapeSelected(controls){return runScrape(controls,true);}
  async function runScrape(controls,selectedOnly){
    if(running){cancelled=true;setStatus(controls,"Stopping after current batch…");return;}running=true;cancelled=false;setBusy(controls,true,selectedOnly?"selected":"scrape");let succeeded=0,failures=0,saved=0,saveFailures=0;
    try{const settings=normalizeSettings(await safeSettings(settingsProvider));const tagger=findVideoTagger(document);const buttons=tagger?findSearchButtons(tagger.list,selectedOnly):[];let cursor=0,retries=[],backoff=0,waitBeforeNext=0,waitMessage="";
      ...
      const action=selectedOnly?"Selected scrape":"Scrape";
      setStatus(controls,cancelled?`Stopped after ${succeeded+failures} of ${buttons.length}.`:`${action} complete: ${succeeded} succeeded${failures?`, ${failures} failed`:""}.${saveSummary}`);
  ...
  return {start,stop,ensureControls,runScrapeAll,runScrapeSelected,runSaveAll};
}
...
function findSearchButtons(list,selectedOnly=false){return[...list.children].filter(row=>!selectedOnly||row.querySelector('button[aria-label="Deselect video"]')).map(row=>[...row.querySelectorAll("button")].find(button=>!button.disabled&&button.querySelector("svg.lucide-search"))).filter(Boolean);}
...
function syncSelectedButton(controls,list){const button=controls.querySelector('[data-action="selected"]');const count=list.querySelectorAll('button[aria-label="Deselect video"]').length;const display=count>0?"":"none",hidden=count>0?"false":"true";if(button.style.display!==display)button.style.display=display;if(button.getAttribute("aria-hidden")!==hidden)button.setAttribute("aria-hidden",hidden);}
function setBusy(controls,busy,active){for(const button of controls.querySelectorAll("button"))button.disabled=busy;const selected=active&&controls.querySelector(`[data-action="${active}"]`);if(selected){selected.disabled=false;setButtonLabel(selected,active==="save"?"Saving…":"Stop");}if(!busy){setButtonLabel(controls.querySelector('[data-action="scrape"]'),"Scrape All");setButtonLabel(controls.querySelector('[data-action="selected"]'),"Scrape Selected");setButtonLabel(controls.querySelector('[data-action="save"]'),"Save All");}}
