...
export function BetterScrapersSettingsPanel() {
  ...
  return <div style={{display:"grid",gap:"1rem",maxWidth:"42rem"}}>
    <style>{`.better-scrapers-primary{display:inline-flex;align-items:center;justify-content:center;gap:.45rem;min-height:2.25rem;border-radius:5px;cursor:pointer;padding:.5rem .85rem;border:1px solid var(--color-accent,#2563eb);background:var(--color-accent,#2563eb);color:#fff;font-weight:650}.better-scrapers-primary:hover{filter:brightness(1.1)}.better-scrapers-primary:disabled{cursor:not-allowed;opacity:.6}`}</style>
    ...
    <label style={{display:"flex",alignItems:"flex-start",gap:".65rem",cursor:"pointer"}}><input type="checkbox" checked={current.useBackoff} onChange={event=>update("useBackoff",event.target.checked)} style={{marginTop:".2rem"}}/><span><strong style={{display:"block"}}>Use backoff</strong><small style={{display:"block",color:"#9ca3af",marginTop:".25rem"}}>When backoff is enabled, if a 429 (too many requests) is encountered during a batch scrape, it will increase the length of the pause until scraping works, up to the maximum backoff.</small></span></label>
    <label style={{display:"grid",gap:".35rem"}}><span style={{fontWeight:600}}>Maximum backoff</span><input type="number" min="1" max="3600" value={current.maximumBackoff} disabled={!current.useBackoff} onChange={event=>update("maximumBackoff",event.target.value)} style={{...inputStyle,opacity:current.useBackoff?1:.55}}/><small style={{color:"#9ca3af"}}>Maximum retry pause in seconds. Defaults to 120.</small></label>
