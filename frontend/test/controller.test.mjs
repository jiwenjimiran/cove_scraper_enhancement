...
test("injects controls in video tagger and hides original",()=>{const {controller,document}=setup();controller.start();assert.equal(document.querySelector('[data-action="save"]').textContent,"Save All");assert.equal(document.getElementById("original").style.display,"none");assert.match(document.getElementById("better-scrapers-styles").textContent,/background:#16a34a/);controller.stop();});
