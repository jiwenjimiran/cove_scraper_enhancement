...
test("injects native-styled controls in video tagger and hides original",()=>{const {controller,document}=setup();controller.start();const scrape=document.querySelector('[data-action="scrape"]'),save=document.querySelector('[data-action="save"]');assert.equal(save.textContent,"Save All");assert.match(scrape.className,/bg-accent/);assert.match(save.className,/bg-green-600/);assert.ok(scrape.querySelector("svg.lucide-cloud-download"));assert.ok(save.querySelector("svg.lucide-check"));assert.equal(document.getElementById("original").style.display,"none");controller.stop();});
